...
- 标签列表中有问题的项会显示为红色
- `Ctrl+Z` 撤销（支持创建/删除/复制/粘贴，快照全量还原，上限 50 步）
- `Ctrl+T` 打开标签统计面板，可双击跳转、勾选多标签跳跃翻页、右键批量重命名
...
- [x] **标签统计面板 — 右键批量重命名**：在统计面板右键标签，统一重命名所有标注文件
- [x] **文件列表右键菜单**：右键图片 → 在文件管理器中定位标注文件（XML/TXT/JSON）
- [x] **历史标签补全**：输入标签时根据历史标签自动弹出候选（大小写不敏感、包含匹配）；打开对话框即展示全部可选
- [x] **撤销栈**：`Ctrl+Z` 撤销（支持创建/删除/复制/粘贴，快照全量还原，上限 50 步）
- [x] **窗口状态持久化**：关闭时记住窗口最大化状态，下次启动恢复
- [x] **快捷键提示面板**：按 `?` / `H` 弹出速查表，展示所有快捷键
- [ ] **批量操作**：多选标签，批量重命名
